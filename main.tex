...
\setuplayout
    [backspace=1in,     % Left margin
     cutspace=1in,      % Right margin
     width=fit,         % Calculate text width from back/cutspace
     margin=0cm,        % Removes extra marginal note area
     edge=0cm]          % Removes extra edge area
...
\setupnotation[footnote][
    numbercommand=\firstofoneargument,
    alternative=left,
    width=2em,
    headalign=flushright,
    hang=1,              % Enables hanging indent
    distance=0.5em,      % Space between number and text
]
...
\setupquotation[
    indenting=no,           % Removes first-line indentation inside the quote
    before={\blank[big]},   % Adds clear space above the blockquote
    after={\blank[big]},    % Adds clear space below the blockquote
    leftmargin=1.5em,       % Standard blockquote indentation from left
    rightmargin=1.5em,      % Standard blockquote indentation from right
]

\setupheadertexts
    [\doifnot\pagenumber1{\tfx\em\documentvariable{title}}]
    [\doifnot\pagenumber1{\tfx\em\documentvariable{author}}]


\setuppagenumbering[location=footer,middle]

\setupdocument
    [title={Communication with Zettelkastens},
     subtitle={An Experiential Report},
     author={Niklas Luhmann},
     translator={Sascha}]

\definecharacterkerning[headertracking][factor=0.1]

\definecharacterkerning[incipittracking][factor=0.03]
\define[1]\incipit{{\setcharacterkerning[incipittracking]{\sc #1}}}

\setuplanguage[en][apostrophe=yes]

\definedelimitedtext[translatorsnote][blockquote]
\setupdelimitedtext[translatorsnote][ leftmargin=1.5pc,
    rightmargin=1.5pc,
    indenting={yes,small},
]
...
\setupinteraction
    [state=start,
     ...
     keyword={zettelkasten, notes, learning},
     color=black,
     contrastcolor=black]

\setupinteractionscreen[option=bookmark]
\placebookmarks[section,subject][force=yes]
...
\startalignment[middle]
    {\blank[2*line] \tfc \documentvariable{title} \blank}
    {\tfb \documentvariable{subtitle} \blank}
    {\blank[line]\tfa \documentvariable{author} \blank}
    {\tf\em Translated by \documentvariable{translator} \blank}
\stopalignment
...
\startsubject[title={Comments on the Translation}]

The original is: Niklas Luhmann (1981): {\em Kommunikation mit Zettelkästen: Ein Erfahrungsbericht}, Opladen: Westdeutscher Verlag.
...
\startsubject[title={Special Thanks}]
